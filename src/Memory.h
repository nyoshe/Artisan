...
#include <span>
#include <stack>
template <class PType, class T> class TempPtr {
private:
...
public:
  ...
  ~TempPtr() { parent->popPtr(); }
  inline auto operator[](usize i) -> T & {
    assert(i < p_size);
    return (*parent)[pos + i];
  };
  inline T &front() { return (*parent)[pos]; }
  ...
  inline void resize(int new_size) { p_size = new_size; }
  inline usize size() const { return p_size; }
  inline bool empty() const { return (p_size == 0); }
...
template <class T, usize Capacity> class SearchArray {
private:
...
public:
  ...
  inline T pop_back() { return std::move(arr[--d_size]); }
...
public:
  ...
  inline usize size() const { return d_size; }
  inline void resize(usize size) { d_size = size; }
  inline bool empty() const { return (d_size == 0); }
  inline void clear() { d_size = 0; }
