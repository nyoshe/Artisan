...
public:
  ...
  constexpr Move(const u8 from, const u8 to, const u8 moved, const u8 capture)
      : Move(from, to, moved) {
    data |= (capture & 0x7) << 15;
  };
  ...
  constexpr Move(const u8 from, const u8 to, const u8 moved, const u8 capture,
                 const u8 promotion)
      : Move(from, to, moved, capture) {
    data |= (promotion & 0x7) << 18;
  };

  constexpr Move(const u8 from, const u8 to, const u8 moved, const u8 capture,
                 const u8 promotion, const bool ep_flag)
      : Move(from, to, moved, capture, promotion) {
  ...
  [[nodiscard]] bool isCastle() const {
    if (piece() == eKing && std::abs(to() - from()) == 2)
      return true;
    return false;
  }
  ...
  void setEnPassant(const bool ep_flag) {
    data |= static_cast<uint32_t>(ep_flag) << 21;
  }
  ...
  [[nodiscard]] explicit constexpr operator bool() const { return data != 0; }
