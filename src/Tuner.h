...
#define TUNABLE_PARAM(name, val, min, max, step)                               \
  inline TunableParam &name##Param =                                           \
      addTunableParam(#name, val, min, max, step);                             \
  inline int name() { return name##Param.value; }

TUNABLE_PARAM(PAWN_CORR_WEIGHT, 186, 64, 2048, 32)

/*error: 0.0604395 {24, 47}
 *{ 1, 48 }
 *{ -26, 73 }
 *{ 7, 18 }
 *{ -6, 6 }
 *{ 45, 76 }
 *{ 12, -1 } { 9, 0 } { 8, 2 } { 3, 5 } { -14, 9 }
 *{ 12, 41 } { 15, 31 } { 20, 35 } { -4, 26 } { -88, 105 }
 *{ 26, -50 }
 */
class Tuner {
public:
  ...
  void updateGradient(double K, double result, double eval,
                      const EvalCounts &counts) {
    int N = pos_vec.size();
    ...
    for (auto &val : gradient) {
      ...
      val[0] +=
          X * reinterpret_cast<const i32 *>(&counts)[i] * (24 - b.getPhase());
