...
// I'm gonna be honest, this was almost entirely AI generated, I can't be
// bothered with parsers
      ...
      if (evalPos != std::string::npos && !moves.empty()) {
        ...
        try {
          if (!evalStr.empty() && evalStr[0] == '#') {
            ...
            int mateDistance =
                std::stoi(evalStr.substr(evalStr[1] == '-' ? 2 : 1));
            moves.back().eval = evalStr[1] == '-' ? -100000 + mateDistance
                                                  : 100000 - mateDistance;
          } else {
            // Handle regular eval score
            moves.back().eval = static_cast<int>(std::stod(evalStr) *
                                                 100); // convert to centipawns
          }
        } catch (...) {
