...
struct BoardParams {
  ...
  int32_t captures[5] = {S(12, 41), S(15, 31), S(20, 35), S(-4, 26),
                         S(-83, 102)};
  int32_t isolated_pawns = S(25, -50);
  // int32_t piece[6] = {S(1,1), S(10 ,20), S(10,20), S(10,20), S(10,20) ,
  // S(10,20)}; int32_t phase_values = S(100,100);
};
...
struct BoardState {
  ...
  BoardState(int ep_square, u8 castle_flags, Move move, int eval, u64 hash,
             u16 half_move)
      : ep_square(ep_square), castle_flags(castle_flags), move(move),
        eval(eval), hash(hash), half_move(half_move) {};
...
inline Zobrist initZobristValues() {
  Zobrist z;
  for (auto &val : z.piece_at)
    val = rnd64();
  z.side = rnd64();
  for (auto &val : z.castle_rights)
    val = rnd64();
  for (auto &val : z.ep_file)
    val = rnd64();
  return z;
}
...
private:
  ...
  int ep_square =
      -1; // -1 means no en passant square, ep square represents piece taken
...
public:
  ...
  boardString() const; // outputs board string in standard ascii
  ...
  // Returns a Move object corresponding to the given UCI string (e.g. "e2e4",
  // "e7e8q").
  ...
  [[nodiscard]] int moveEstimatedValue(Move move);
  ...
  [[nodiscard]] int getPhase() const {
    return 24 - BB::popcnt(boards[eWhite][eKnight]) -
